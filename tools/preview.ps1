<#
  ...
  Presenting from another device — a phone, the meeting laptop, the projector PC?
  Add -Lan and the report becomes reachable from anywhere on the same network:

      powershell -File tools\preview.ps1 -Lan

  It will print the address to type. Without -Lan the report is reachable only from
  this computer, which is the safe default: a month that has not been published yet
  should not be readable by everyone on the coop network unless you meant it to be.
#>
...
$bindTo = if ($Lan) { '0.0.0.0' } else { '127.0.0.1' }
...
if ($Lan) {
    ...
    foreach ($ip in $ips) {
        Write-Host "  From another device on this network:  http://$($ip.IPAddress):$Port/"
    }
    Write-Host ""
    Write-Host "  ANYONE on this network can now read this report. Close this window when you finish." -ForegroundColor Yellow
}
...
}).listen(port, process.argv[4] || '127.0.0.1');
'@

$tmp = Join-Path $env:TEMP 'bugemco-preview-server.js'
$server | Out-File -FilePath $tmp -Encoding utf8
try { & node $tmp $root $Port $bindTo } finally { Remove-Item $tmp -ErrorAction SilentlyContinue }
