...
test('targetBar prints the true percentage when the target is beaten, and still caps the bar', () => {
  // The bar is capped because a groove has a length. The number is not, because
  // 11 of 10 is 110% and printing "100%" beside it contradicts the figure.
  const svg = targetBar({ label: 'System improvements proposed', actual: 11, target: 10 });
  assert.match(svg, /width="100%"/);
  assert.ok(svg.includes('110%'));
  assert.ok(svg.includes('11 of 10'));
  assert.ok(svg.includes('data-state="ahead"'));
  // A target exactly met is 'met', not 'ahead' — otherwise 5 of 5 and 11 of 10
  // paint the same and the state says nothing.
  const met = targetBar({ label: 'Digital agreements standardised', actual: 5, target: 5 });
  assert.ok(met.includes('data-state="met"'));
});

test('targetBar calls nothing behind unless the pace is given', () => {
  const without = targetBar({ label: 'Information drives held', actual: 4, target: 5 });
  assert.ok(without.includes('data-state="on-track"'));
  assert.ok(!without.includes('--pace'));
  const with67 = targetBar({ label: 'Digitalisation projects in use', actual: 3, target: 6, pace: 67 });
  assert.ok(with67.includes('data-state="behind"'));
  // The pace has to reach CSS, or a marker cannot be drawn at it.
  assert.ok(with67.includes('--pace:67%'));
});
