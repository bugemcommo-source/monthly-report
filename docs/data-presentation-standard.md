...
Two of July's captures could not be published as taken, and **both have since been
withdrawn from the report altogether** — a crop removes what you can see and leaves the
question of what the rest of the screen is drawn from. The reasons are recorded in
`tracking/projects/mart-online-store.md` and `tracking/projects/cac-iis.md`.

**The lesson worth keeping:** if a screen needs a crop to be publishable, ask first whether
it should be published at all. The system's work is almost always described better in words
than in a picture of it.

Originals stay in the private folder, which is gitignored, so nothing is lost and the
untouched versions never reach the public repository.
...
opened from the keyboard, or if a redacted region or an email address reaches the page
text. Run it before any month is published.
